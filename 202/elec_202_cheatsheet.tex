...
\end{defin}
See figure \ref{fig:lapace_tb_1} and \ref{fig:lapace_tb_2 } for some transformation and proprieties. \\

\textbf{Solve inverse laplace}: For transfer function or general F(s), use partial fraction to simplify each terms and look up it's inverse from table \ref{fig:lapace_tb_1}. What am I kidding just use HP Prime. 

\subsubsection{Laplace transform of circuit} \label{sec:lap_domain}
\begin{itemize}[noitemsep,nolistsep]
    \item $Z(s) = R$
    \item $Z(s) = sL$
    \item $Z(s) = \frac{1}{sC}$
...
% In \ref{sec:lap_domain} we analysis the laplace domain, for frequency domain, we use Fourier Transform and replace $s = \imath \omega$ 
...
\subsection{Construct and Analysis of Phase Plot}
\begin{defin}
...
\subsection{Sallen Key configuration}
...
\begin{itemize}[noitemsep,nolistsep]
    \item determine $x(0),\frac{\mathrm{d}x}{\mathrm{d}t},x(\infty) $
    \item find $x_t(t)$ by turning off independent source, plug in initial condition $x(0),\frac{\mathrm{d}x}{\mathrm{d}t}$.
    \item $x(t) = x_t(t) + x_{ss}(t)$, where $x_{ss}(t)$ is $x(\infty)$.
\end{itemize}

\begin{question}
Given a circuit, solve for $v(t), i(t)$
\end{question}
